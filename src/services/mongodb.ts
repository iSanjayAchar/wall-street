 ...
 */
const collections = (await connection.db(Config.database.mongodb.dbName).listCollections().toArray()).map(
...
async function Write(pair = Config.pair.default, timestamp = new Date(), price = 0) {
  try {
